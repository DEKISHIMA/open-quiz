...
io.on('connection',s=>{
  s.on('create',()=>{let code;do code=makeCode();while(rooms.has(code));const hostToken=makeToken();const r={host:s.id,hostToken,questions:[{text:'日本で一番高い山は？',image:''}],current:1,phase:'setup',players:new Map(),results:new Map(),remaining:90,timer:null,cleanupAt:0};rooms.set(code,r);s.data.room=code;s.data.role='host';s.data.hostToken=hostToken;s.join(code);s.emit('created',{code,token:hostToken});emitState(code)});
  s.on('resumeHost',d=>{const code=String(d?.code||'').toUpperCase(),r=rooms.get(code);if(!r||r.hostToken!==d?.token)return s.emit('err','司会者の再接続情報が見つかりません。');r.host=s.id;s.data.room=code;s.data.role='host';s.data.hostToken=d.token;s.join(code);r.cleanupAt=0;emitState(code);if(r.phase==='answer'&&r.remaining>0)startTimer(code,false);s.emit('resumedHost')});
  s.on('joinDisplay',d=>{const code=String(d?.roomCode||'').trim().toUpperCase(),r=rooms.get(code);if(!r)return s.emit('displayError','部屋がありません。');s.data.room=code;s.data.role='display';s.join(code);s.emit('displayJoined',{code});s.emit('state',statePayload(r))});
  ...
  s.on('questions',qs=>{const r=rooms.get(s.data.room);if(!r||r.host!==s.id||r.phase!=='setup')return;const a=Array.isArray(qs)?qs.map(normalizeQuestion).filter((q,i)=>i<20&&q.text.trim()):[];if(!a.length)return s.emit('err','1問以上入力してください。');r.questions=a;r.current=1;r.results=new Map();newQuestion(s.data.room)});
